...
For any motif in species $i$’s motif census ($m_{i,x} \in M_i$), we evaluate the net effect of all stressors on the three species as the sum of their direct effects on all species involved -- that is, their species-specific responses $S_{m_{i,x}}$ -- as in @halpern2008a:

$$S_{m_{i,x}} = \sum_{a \in m_{i,x}} \sum_j D_j * \mu_{a,j} \text{,} \label{eq1} \tag{1}$$


where $a$ are the species involved in motif $m_{i,x}$, $D_j$ is the standardized intensity of stressor $j$ (scaled between 0 and 1; see Stressors data) and $\mu_{a,j}$ is the relative species-specific sensitivity of species $a$ to stressor $j$ (range between 0 and 1; see Vulnerability). The relative contribution of stressors to direct effects are those that affect focal species $i$, whereas the relative contribution to indirect effects are those that affect the species interacting with species $i$ in $m_{i,x}$.
...
where $C_{i,m_{i,x}}$ is the cumulative effect of all stressors on species $i$ through interaction $m_{i,x}$, and $T_{i,m_{i,x}}$ is the trophic sensitivity of species $i$ to the effects of stressors through interaction $m_{i,x}$. We then summarize cumulative effects on species $i$ by averaging the cumulative effects across a species’ motif census to obtain an unbiased measure of the topological configuration of species interactions [@stouffer2012]:

$$C_{N_{i,x}} = \frac{1}{|M_i|} \sum_{m _{i,x} \in M_i} C_{i,m_{i,x}} \text{,} \label{eq3} \tag{3}$$
...
\caption{Comparison of the network-scale and species-scale cumulative effects assessments for a) sea cucumbers (\textit{Cucumaria frondosa}), b) monkfish (\textit{Lophius americanus}) and c) beluga whales (\textit{Delphinapterus leucas}).}
\label{ch5-species}
\end{figure}
