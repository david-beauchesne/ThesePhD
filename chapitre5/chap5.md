...
\caption{\textbf{Comparison of network-scale and species-scale cumulative effects for individual species.} Scatterplot of the mean network-scale cumulative effects as a function of the species-scale cumulative effect over a species predicted distribution ($C / km^2$) for all 193 taxa considered. The size of the point is the trophic sensitivity of each taxon, corresponding to the scaled trophic sensitivity given a species’ position in a food web and stressors entry points \citep{beauchesne2020b}. Taxa along the orange line are those for which the network-scale and species-scale assessments are equal, that is species which have no predicted interaction.}
...
$$S_{m_{i,x}} = \sum_{a \in m_{i,x}} \sum_j D_j * \mu_{a,j} \text{,} \label{eq1} \tag{5.1}$$
...
$$C_{i,m_{i,x}} = T_{i,m_{i,x}} * S_{m_{i,x}} \text{,} \label{eq2} \tag{5.2}$$
...
$$C_N = \sum_i \frac{1}{|M_i|} \sum_{m_{i,x} \in M_i} T_{i,m_{i,x}} * \sum_{a \in m_{i,x}} \sum_j D_j * \mu_{a,j} \label{eq5} \tag{5.5}$$
...
\tabularnewline
 ...
 btb & 0.1.30 & CRAN &  \citet{dossantos2018} \\
 eDrivers & 0.0.1 & GitHub & \citet{beauchesne2020a} \\
 fasterize & 1.0.0 & CRAN & \citet{ross2018} \\
 kableExtra & 1.1.0 & CRAN & \citet{zhu2019} \\
 magrittr & 1.5 & CRAN & \citet{bache2014} \\
 motifcensus & 0.0.2.9000 & GitHub & \citet{cazelles2020} \\
 randomForest & 4.6.14 & CRAN & \citet{liaw2002} \\
 raster & 2.8.19 & CRAN & \citet{hijmans2019} \\
 rfishbase & 3.0.3 & CRAN & \citet{boettiger2012} \\
 rglobi & 0.2.19 & CRAN & \citet{poelen2019} \\
 rmarkdown & 2.1 & CRAN & \citet{xie2018, allaire2020} \\
 sf & 0.7.4 & CRAN & \citet{pebesma2018} \\
 sp & 1.3.1 & CRAN & \citet{pebesma2005}, \citet{bivand2013} \\
 taxize & 0.9.8 & CRAN & \citet{chamberlain2013}, \citet{chamberlain2019} \\
 tidyverse & 1.2.1 & CRAN & \citet{wickham2017} \\
 worrms & 0.4.0.98 & CRAN & \citet{chamberlain2020} \\
 \bottomrule
...
\caption{Distribution and intensity of stressors in the St. Lawrence system. The cumulative exposure of groups of stressors are evaluated as the sum of the scaled intensities of individual stressors in that group \citep{beauchesne2020}.}
\label{ch5-stressors}
\end{figure}
...
\caption{Regional metrics for stressors and ecological communities in the St. Lawrence System. Spatial distribution of the cumulative exposure (\textit{i.e.} sum of normalized driver intensity in each grid cell) arising from the 18 stressors considered (a), taxa richness (b), link density (c) and connectance (d). Network-scale cumulative effects and transgressive properties arising from species interactions (\textit{i.e.} the sum of individual species log ratio between the network-scale and species-scale cumulative effects assessments) as a function of cumulative exposure (e,i), taxa richness (f,j), link density (g,k) and connectance (h,l).}
\label{ch5-metrics}
\end{figure}
